...
function domLoaded() {
    ...
    /* Pretty print of for loop:
        ...
        if (booksInfo[i].isInReadingList == false) {
            ...
            addButtoni.addEventListener('click', function() {
                ...
                window.location.href='home-page.php?id=' + id;
            });
        };
    ...
    */
    for(i = 0; i < booksInfo.length; i++) {
        ...
        eval("if (booksInfo[" + i + "].isInReadingList == false) { var addButton" + i + " = document.createElement('button'); addButton" + i + ".classList.add('add-to-reading-list-button'); addButton" + i + ".innerText = '+'; book" + i + ".append(addButton" + i + "); addButton" + i + ".addEventListener('click', function() { var id = booksInfo[" + i + "].id; window.location.href='home-page.php?id=' + id; }); };");
